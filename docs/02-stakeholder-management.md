...
```mermaid
flowchart TD
    KE["Kelly Electric\n(Derek Newman - primary, John Arnold - secondary)\nProvides data & site access"]
    SA["Solar Advocates\n(David Brosch, Steve Skolnik, Albert Nunez)\nSet project goals & direction"]
    Team["Student Team\n(Rhea - liaison)\nData collection & analysis"]
    Util["Utilities / County\n(BGE, Miss Utility, MD utilities)\nSite & infrastructure support"]

    SA -->|goals & alignment| Team
    KE -->|data & system access| Team
    Team -->|updates, questions, findings| KE
    Team -->|updates, questions, findings| SA
    Util -->|site coordination| Team
```
